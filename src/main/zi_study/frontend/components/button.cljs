...
(defn button
  ...
  (let [[opts children] (if (and (= 1 (count args)) (not (map? (first args))))
        ...
        {:keys [variant size full-width start-icon end-icon disabled class on-click]
         :or {variant :primary
              size :md
              full-width false
              disabled false}} opts
        ...
        attrs (-> (dissoc opts :variant :size :full-width :start-icon :end-icon :disabled :class :on-click)
